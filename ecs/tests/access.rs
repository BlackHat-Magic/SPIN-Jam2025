...
fn component_access_detects_conflicting_writes() {
    ...
    let a = ComponentAccess { read: &READ, write: EMPTY };
    let b = ComponentAccess { read: EMPTY, write: &OTHER_WRITE };
    let c = ComponentAccess { read: EMPTY, write: &WRITE };
...
fn component_access_detects_read_write_conflicts() {
    ...
    let reader = ComponentAccess { read: &READ, write: EMPTY };
    let writer = ComponentAccess { read: EMPTY, write: &WRITE };
...
fn resource_access_mirrors_component_access_rules() {
    ...
    let read_only = ResourceAccess { read: &READ, write: EMPTY };
    let write_only = ResourceAccess { read: EMPTY, write: &WRITE };
    let disjoint_write = ResourceAccess { read: EMPTY, write: &[9] };
